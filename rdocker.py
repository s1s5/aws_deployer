...
def create_rc_file(hosts, fp):
    ...
    if len(hosts) == 1:
        ...
        print_('export DOCKER_HOST={}'.format(get_sock(hosts[0])), file=fp)
        print_('docker -> [{}]'.format(hosts[0]))
    else:
        common = search_commons(hosts)
        print_(r'export PS1="(rdocker) $PS1"', file=fp)
        for index, host in enumerate(hosts):
            print_("alias docker_{}='docker -H {}'".format(host[len(common):], get_sock(host)), file=fp)
            print_("alias d{}='docker -H {}'".format(host[len(common):], get_sock(host)), file=fp)
